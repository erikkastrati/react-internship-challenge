...
const userSchema = z.object({
  ...
  addressStreet: z.string().min(2).optional(),
...
export default function AddUser({ users, setUsers }) {
  ...
  const onSubmit = (data) => {
    const newUser = {
      ...
      company: { name: data.company?.trim() || "" },
      address: {
  ...
  return (
    <div className="max-w-md mx-auto mt-10 bg-black text-white p-6 rounded-lg shadow relative">
      {notification && (
        <div className="fixed top-20 right-5 bg-green-600 text-white px-4 py-2 rounded shadow-lg transition-all">
          {notification}
        </div>
      )}
      <h1
        className={`text-3xl font-bold mb-6 flex items-center gap-2 ${textStyle}`}>
